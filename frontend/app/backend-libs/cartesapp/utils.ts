...
export async function ensureCartesiPublicClient(
  ...
  if (params.client) {
    ...
    client = await getL2Client(params.cartesiNodeUrl + "/rpc");
    if (!client)
      throw new Error(
        "Couldn't get Cartesi Public Client: No node url defined.",
      );
...
export interface MutationOptions extends BaseLayerWalletOptions {
  decode?: boolean;
  sync?: boolean;
  cartesiNodeUrl?: string;
  initialDelay?: number;
...
export class IOData<T extends object> {
  ...
  validate = (): boolean => {
    ...
    for (const k of Object.keys(dataToValidate)) {
      if (
        this._model.ioType == "mutationPayload" &&
        (typeof dataToValidate[k] == "bigint" ||
          typeof dataToValidate[k] == "number")
      )
        dataToValidate[k] = toHex(
          dataToValidate[k] < BigInt(0) ? -dataToValidate[k] : dataToValidate[k],
        );
    }
  ...
  export(selectorInfo?: string): string {
    ...
    switch (this._model.ioType) {
      ...
      case "queryPayload":
      case "queryJsonPayload":
      case "queryJsonrpcPayload": {
        // parametrize input to url
        ...
        if (this._model.ioType == "queryJsonrpcPayload") {
          jsonObj["jsonrpc"] = "2.0";
...
export async function inspectCall(
  ...
  const inspectResult = await inspect(payload, options).catch((e) => {
...
export async function genericInspect<T extends object>(
  ...
  if (options == undefined) throw new Error("No options defined");
...
export function genericDecodeTo<T extends object>(
  ...
  switch (model.ioType) {
    ...
    case "notice": {
      ...
      for (const k of Object.keys(dataToValidate)) {
        if (typeof dataToValidate[k] == "object") {
            for (const dk in dataToValidate[k]) {
            if (typeof dataToValidate[k][dk] == "bigint")
                dataToValidate[k][dk] = toHex(
                dataToValidate[k][dk] < BigInt(0)
                    ? -dataToValidate[k][dk]
                    : dataToValidate[k][dk],
                );
            }
        }
        if (typeof dataToValidate[k] == "bigint")
        dataToValidate[k] = toHex(
          dataToValidate[k] < BigInt(0) ? -dataToValidate[k] : dataToValidate[k],
        );
      }
